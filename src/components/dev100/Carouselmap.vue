...
nextTick(() => {
  const slideContainer = document.querySelector('ul') as HTMLElement
  // eslint-disable-next-line max-statements-per-line
  setInterval(() => { slide(slideContainer) }, 3000)
})
</script>
  ...
  <dev-layout title="Carousel Map">
    <div class="w-300 h-300 relative overflow-hidden">
      <ul class="flex absolute list-none">
        <li v-for="img, index in imgUrls" :key="index" class="list-none">
